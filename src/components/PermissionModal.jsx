...
export const PermissionModal = ({
    ...
    if (cameraEnabled) {
      ...
      try {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          throw new Error("Your browser does not support the navigator.mediaDevices.getUserMedia API");
        }
        ...
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
          setCameraError("Camera permission blocked. Please click the 🔒 or 📹 icon in your browser address bar (URL) and select 'Allow', then retry.");
        } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
          setCameraError("No webcam device found on your computer. Please check your hardware connection.");
        } else if (err.name === 'NotReadableError' || err.name === 'TrackStartError') {
          setCameraError("Webcam is currently in use by another application (Zoom, Teams, Meet...). Please close that app and retry.");
        } else {
          setCameraError(err.message || "Failed to connect to webcam.");
        }
      } finally {
  ...
  const handleToggleMic = async () => {
    ...
    if (micEnabled) {
      ...
      try {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          throw new Error("Browser does not support navigator.mediaDevices.getUserMedia API");
        }
        const stream = await navigator.mediaDevices.getUserMedia({ 
          audio: {
        ...
        setMicEnabled(false);
        if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
          setMicError("Microphone permission blocked. Please click the 🔒 or 🎙️ icon in your browser address bar (URL) and select 'Allow', then retry.");
        } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
          setMicError("No microphone found on your computer.");
        } else {
          setMicError(err.message || "Failed to connect to microphone.");
        }
      } finally {
              ...
              <div className="flex items-start space-x-3.5">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <h4 className="text-sm font-semibold text-slate-100">Physical WebCam (Camera)</h4>
                    {cameraEnabled ? (
            ...
            {cameraError && (
              <div className="mt-3 p-3 bg-red-950/60 border border-red-500/40 rounded-lg text-xs text-red-300 space-y-2">
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-400">Please click 'Allow' on the browser prompt</span>
                  <button
                    onClick={handleToggleCamera}
              ...
              <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center space-x-3">
                ...
                <div className="text-xs font-mono text-slate-400 space-y-0.5">
                  <div className="text-emerald-400 font-semibold flex items-center space-x-1">
                    ...
                    <span>WebCam is active</span>
                  </div>
                  <div className="text-[11px] text-slate-500">Live Frame Feed | In-Memory Local Stream</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className={`p-4 rounded-xl border transition-all ${
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-3.5">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <h4 className="text-sm font-semibold text-slate-100">Physical Microphone</h4>
                    {micEnabled ? (
                  ...
                  <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                    Directly measures room noise in decibels (dB) to automatically activate acoustic masking when exceeding 65dB.
                  </p>
                </div>
              </div>
            ...
            {micError && (
              <div className="mt-3 p-3 bg-red-950/60 border border-red-500/40 rounded-lg text-xs text-red-300 space-y-2">
                ...
                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-400">Please click 'Allow' for microphone on your browser</span>
                  <button
                    onClick={handleToggleMic}
                    className="px-2.5 py-1 bg-red-800/60 hover:bg-red-700 text-red-200 rounded border border-red-500/30 text-[11px] font-semibold flex items-center space-x-1 cursor-pointer"
                  >
              ...
              <div className="mt-3 pt-3 border-t border-slate-800/80 space-y-1.5">
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-slate-400 flex items-center space-x-1">
                    ...
                    <span>Live Microphone Level Test:</span>
                  </span>
