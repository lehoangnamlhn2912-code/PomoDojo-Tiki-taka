...
export class PoseDetector {
  ...
  classifyExercise(lm) {
    ...
    if (isLateralRaise) {
      return { exercise: 'lateral_raise', name: 'Lateral Raise (Arms out)', confidence: 0.9, metrics: { lShoulderAngle, rShoulderAngle } };
    }
    if (isHammerCurl) {
      return { exercise: 'hammer_curl', name: 'Hammer Curl (Bicep curl)', confidence: 0.9, metrics: { lElbowAngle, rElbowAngle } };
    }
    if (isSquat) {
      return { exercise: 'squat', name: 'Squat (Deep squat)', confidence: 0.88, metrics: { lKneeAngle, rKneeAngle } };
    }
    if (isPushUp) {
